...
Shader::Shader(const std::string &filePath): Resource(filePath)
{
	...
	else if (EndsWith(filePath, ".compute"))
	{
...
void Shader::Init(const string& filePath)
{
	...
	shaderName = filePath.substr(pos + 1);
...
void Shader::CompileShaderFromSource(const string& source)
{
	...
	if (compileStatus != GL_TRUE)
	{
		...
		std::cout << shaderName << "ERROR:: Compile Shader FromSource failed!" << infoLog << std::endl;
	}else
	{
		std::cout << shaderName << shaderID <<" shader compile success!"<< std::endl;
	}
}
void Shader::Destroy()
{
