...
/*
	...
	static inline Color MakeColor(const double* c)
	{
		if (c != NULL && c + 1 != NULL && c + 2 != NULL)
		{
			if (c + 3 != NULL)
			{
				return Color(*c, *(c + 1), *(c + 2), *(c + 3));
			}
			else
			{
				return Color(*c, *(c + 1), *(c + 2), 1);
			}
		}
		return Color::white;
	}

	static inline Color MakeColor(const float* c)
	{
		if (c != NULL && c + 1 != NULL && c + 2 != NULL)
		{
			if (c + 3 != NULL)
