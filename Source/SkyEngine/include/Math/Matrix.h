...
public:
	...
	void setBasis(const Vector3& inX, const Vector3& inY, const Vector3& inZ);
	...
	float& Get(int row, int column) { return m[row + (column * 3)]; }
	const float& Get(int row, int column) const { return m[row + (column * 3)]; }


	static bool LookRotationToMatrix(const Vector3& viewVec, const Vector3& upVec, Matrix3* m);
...
public:
	...
	//Matrix4x4 FPSView(const Vector3& eye, Quaternion rotation);
...
inline void Matrix3::setBasis(const Vector3& inX, const Vector3& inY, const Vector3& inZ)
{
	m[0] = inX.x;  m[1] = inY.x;  m[2] = inZ.x;
	m[3] = inX.y;  m[4] = inY.y;  m[5] = inZ.y;
	m[6] = inX.z;  m[7] = inY.z;  m[8] = inZ.z;
}
