...
bool App::CreateWorld()
{
	...
	bunny = new Model("../../BuiltinAssets/model/sponza/sponza.obj", true);
	bunny->CreateBufferData();

	sponza = new GameObject();
	vector<Mesh>* meshes = bunny->GetMeshes();
	refractionShader = new ShaderProgram("./assets/lighting.vert", "./assets/texture2.frag");
	//shaderProgram = new ShaderProgram("./assets/lighting.vert", "./assets/lighting.frag");
